...
import { useCitizenContext } from '@/lib/context/CitizenContext';



const CitizenManagement = () => {
  const { citizens, insertCitizen, deleteCitizen, updateCitizen, getAllCitizen } = useCitizenContext();
  ...
  // Citizen data array
  const [citizensBackup, setCitizensBackup] = useState<Citizen[]>(citizens);

  React.useEffect(() => {
    setCitizensBackup(citizens);
  }, [citizens]);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const success = insertCitizen(formData);
  ...
  const handleDelete = (nic: string) => {
    ...
    const success = deleteCitizen(nic);
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (e.target.value === '') {
      ...
      const filteredCitizens = citizens.filter((citizen) => citizen.nic.includes(e.target.value));
      setCitizensBackup(filteredCitizens);
    }
